	...
	if (user->get_priority() == core::UP_OPERATOR) {
		...
		if (m_machine) {
			if (!m_machine->get_is_submachine()) {
				...
				if (is_machine_can_link_submachine(m_machine->get_machine_type()))
...
BOOL CAlarmMachineDlg::OnInitDialog()
{
	...
	SET_WINDOW_TEXT(IDC_BUTTON_EDIT_DETECTOR, IDS_STRING_IDC_SET_PROBE_ICON);	//设置探头图标
	SET_WINDOW_TEXT(IDC_BUTTON_MGR_CAMERA_ICON, IDS_STRING_IDC_BUTTON_MGR_CAMERA_ICON);	
	SET_WINDOW_TEXT(IDC_BUTTON_EDIT_VIDEO, IDS_STRING_IDC_BUTTON_EDIT_VIDEO);
	SET_WINDOW_TEXT(IDC_STATIC_HISTORY, IDS_STRING_IDC_HISTORICAL_RECORD);			//设置历史纪录
	SET_WINDOW_TEXT(IDC_BUTTON_MORE_HR, IDS_STRING_IDC_BUTTON_MORE_HR);		//查看详情
	//SET_WINDOW_TEXT(IDC_STATIC_CONTENT, IDS_STRING_IDC_BUTTON_EDIT_VIDEO);			//报警提示
	SET_WINDOW_TEXT(IDC_STATIC_CONTROL_PANEL, IDS_STRING_IDC_STATIC_CONTROL_PANEL);
	...
	if (m_machine->get_is_submachine()) {
		text = TR(IDS_STRING_SLAVE_CONN);
		m_staticConn.SetWindowTextW(text);
		smachine = TR(IDS_STRING_SUBMACHINE);
		m_btnManageExpire.EnableWindow(0);
	} else {
...
void CAlarmMachineDlg::UpdateCaption()
{
	...
	if (m_machine->get_is_submachine()) {
		...
		sid.Format(L"%s" + TR(IDS_STRING_FM_ADEMCO_ID), fmMachine, m_machine->get_ademco_id());
	}
...
void CAlarmMachineDlg::LoadMaps()
{
	...
	if (!m_machine->get_is_submachine() && m_machine->get_machine_type() == MT_NETMOD) { // sub machines
		CString sAllSubMachine; sAllSubMachine = TR(IDS_STRING_ALL_SUBMACHINE);
...
void CAlarmMachineDlg::OnBnClickedButton1()
{
	...
	if (bsubmachine) {
		...
		if (m_machine->get_submachine_count() == 0) {
			CString e; e = TR(IDS_STRING_E_MACHINE_NO_SUB);
	...
	if (bsubmachine) {
		...
		if (m_machine->get_zone_count() == 0) {
			...
			if (wire_zone_cnt == m_machine->get_zone_count()) {
				CString e; e = TR(IDS_STRING_WIRE_ZONE_NO_NEED_RESTORE);
				MessageBox(e, L"", MB_ICONINFORMATION);
				return;
			}
		}
	...
	if (TIMER_ID_TRAVERSE_ADEMCO_LIST == nIDEvent) {
		...
		if (--m_nRemoteControlTimeCounter > 0) {
			...
			if (EVENT_QUERY_SUB_MACHINE == m_curRemoteControlCommand) {
				CString e; e = TR(IDS_STRING_QUERY_FAILED);
...
void CAlarmMachineDlg::OnBnClickedButtonEditZone()
{
	...
	while (!alarm_machine_manager::get_instance()->EnterBufferMode()) {
		if (GetTickCount() - start > 3000) {
			CString e; e = TR(IDS_STRING_MACHINE_BUSY);
	...
	if (dlg.m_bNeedReloadMaps)
		LoadMaps();
	while (!alarm_machine_manager::get_instance()->LeaveBufferMode()) { std::this_thread::sleep_for(std::chrono::milliseconds(500)); }
}
