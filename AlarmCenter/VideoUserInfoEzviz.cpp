...
bool video_user_info_ezviz::execute_set_acc_token(const std::string& accToken)
{
	...
	USES_CONVERSION;
	CString sql; 
	sql.Format(L"update user_info_ezviz set access_token='%s' where ID=%d",
...
bool video_user_info_ezviz::execute_set_token_time(const std::chrono::system_clock::time_point& tp)
{
	AUTO_LOG_FUNCTION;
	CString sql;
	sql.Format(L"update user_info_ezviz set tokenTime='%s' where ID=%d", time_point_to_wstring(tp).c_str(), real_user_id_);
	if (video_manager::GetInstance()->Execute(sql)) {
		set_token_time(tp);
		return true;
	}
