...
HBITMAP aarot::dorotate(HBITMAP src, double rotation, aar_callback callbackfunc, int bgcolor, bool autoblend, int cntPt, CPoint *pts)
{
	...
	{
		double ox = -srcbmp.bmWidth / 2;
		double oy = -srcbmp.bmHeight / 2;

		pts[i].x = static_cast<long>(coss * ox - sins * oy); 
		pts[i].x += width / 2; 
		
		pts[i].y = static_cast<long>(coss * oy + sins * ox);
		pts[i].y += height / 2;
	}
            ...
            for (int xx = mindx; xx < maxdx; xx++)
            {
                for (int yy = mindy; yy < maxdy; yy++)
                {
                    ...
                    if (dbloverlap)
                    {
						...
						if(0xff == srcdib[SrcIndex].rgbRed
							&& 0x00 == srcdib[SrcIndex].rgbGreen
							&& 0x00 == srcdib[SrcIndex].rgbBlue)
						{
							dbldstdib[DstIndex].red = (srcdib[SrcIndex].rgbRed);
							dbldstdib[DstIndex].blue = (srcdib[SrcIndex].rgbBlue);
							dbldstdib[DstIndex].green = (srcdib[SrcIndex].rgbGreen);
    ...
    RGBQUAD backcolor;
    backcolor.rgbRed = static_cast<unsigned char>(bgcolor & 0x000000FF);
    backcolor.rgbGreen = static_cast<unsigned char>((bgcolor & 0x0000FF00) / 0x00000100);
    backcolor.rgbBlue = static_cast<unsigned char>((bgcolor & 0x00FF0000) / 0x00010000);
...
HBITMAP aarot::rotate(HBITMAP src, double rotation, aar_callback callbackfunc, int bgcolor, bool autoblend, int cntPt, CPoint *pts)
{
    ...
    HBITMAP res = dorotate(src, rotation, callbackfunc, bgcolor, autoblend, cntPt, pts);
