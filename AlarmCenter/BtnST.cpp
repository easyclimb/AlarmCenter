...
#define	TTS_BALLOON		0x40
	...
	void CButtonSTUrlLike::PreSubclassWindow()
	{
	...
	{
		/*
		CDC*    pPsDC = CDC::FromHandle(lpDIS->hDC);
		CDC     memDC;
		memDC.CreateCompatibleDC(pPsDC);
		CDC*	pDC = &memDC;
		*/
		...
		//pPsDC->BitBlt(0, 0, lpDIS->rcItem.right - lpDIS->rcItem.left, 
		//	lpDIS->rcItem.bottom - lpDIS->rcItem.top, pDC, 0, 0, SRCCOPY);
	} // End of DrawItem
	...
	DWORD CButtonST::OffsetColor(BYTE byColorIndex, short shOffset, BOOL bRepaint)
	{
		...
		if (shOffset > 0) {
			if (byRed + shOffset > 255)		shOffsetR = static_cast<short>(255 - byRed);
			if (byGreen + shOffset > 255)	shOffsetG = static_cast<short>(255 - byGreen);
			if (byBlue + shOffset > 255)	shOffsetB = static_cast<short>(255 - byBlue);

			shOffset = min(min(shOffsetR, shOffsetG), shOffsetB);
		} // if
		else {
			if (byRed + shOffset < 0)		shOffsetR = static_cast<short>(-byRed);
			if (byGreen + shOffset < 0)		shOffsetG = static_cast<short>(-byGreen);
			if (byBlue + shOffset < 0)		shOffsetB = static_cast<short>(-byBlue);
	...
	{
		BYTE	byIndex = static_cast<BYTE>(bPlayOnClick ? 1 : 0);
