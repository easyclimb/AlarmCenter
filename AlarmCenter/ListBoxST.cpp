...
void CListBoxST::DrawItem(LPDRAWITEMSTRUCT pDIStruct)
{
	...
	// Calculate rcIcon
	if (m_pImageList) {
		rcIcon.left += CX_BORDER;
		rcIcon.top += CY_BORDER;
		rcIcon.right = rcIcon.left + m_szImage.cx + CX_BORDER * 2;
		rcIcon.bottom = rcIcon.top + m_szImage.cy + CY_BORDER * 2;
		if (bIsDoubleDetector) {
			rcIcon2.left = rcIcon.right;
			rcIcon2.right = rcIcon2.left + rcIcon.Width();
		} else {
			rcIcon2.SetRect(0, 0, 0, 0);
		}
	} // if
	else {
	...
	if (m_pImageList)
		OnDrawIcon(pDIStruct->itemID, pDC, &rcItem, &rcIcon, lpLBData->nImage, 
					bIsDoubleDetector, bIsDisabled, bIsSelected);
...
DWORD CListBoxST::OnDrawIconBackground(int nIndex, CDC* pDC, CRect* prcItem, CRect* prcIcon, BOOL bIsDisabled, BOOL bIsSelected, COLORREF crSuggestedColor)
{
	...
	pDC->FillSolidRect(prcItem->left, prcItem->top,
						prcItem->Width(), prcItem->Height(), crSuggestedColor);
	UNREFERENCED_PARAMETER(nIndex);
	UNREFERENCED_PARAMETER(pDC);
	UNREFERENCED_PARAMETER(prcItem);
	UNREFERENCED_PARAMETER(prcIcon);
	UNREFERENCED_PARAMETER(bIsDisabled);
	UNREFERENCED_PARAMETER(bIsSelected);
	UNREFERENCED_PARAMETER(crSuggestedColor);
	return 0;
...
DWORD CListBoxST::OnDrawTextBackground(int nIndex, CDC* pDC, CRect* prcItem, CRect* prcText, BOOL bIsDisabled, BOOL bIsSelected, COLORREF crSuggestedColor)
{
	...
	UNREFERENCED_PARAMETER(pDC);
	UNREFERENCED_PARAMETER(prcItem);
	UNREFERENCED_PARAMETER(prcText);
	UNREFERENCED_PARAMETER(bIsDisabled);
	UNREFERENCED_PARAMETER(bIsSelected);
	pDC->SetBkColor(crSuggestedColor);
...
DWORD CListBoxST::OnDrawIcon(int nIndex, CDC* pDC, CRect* prcItem, CRect* prcIcon, int nImage, BOOL bIsDouble, BOOL bIsDisabled, BOOL bIsSelected)
{
	...
	*/
	...
	UNREFERENCED_PARAMETER(bIsDisabled);
	UNREFERENCED_PARAMETER(bIsSelected);
	m_pImageList->Draw(pDC, nImage, CPoint(prcIcon->left + 2, prcIcon->top + 2), ILD_TRANSPARENT);
	if (bIsDouble && m_pImageList2) {
		m_pImageList2->Draw(pDC, nImage,
							CPoint(prcIcon->left + 2 + prcIcon->Width(), prcIcon->top + 2),
							ILD_TRANSPARENT);
	}
	return 0;
} // End of OnDrawIcon
...
{
	UINT			nIndex = static_cast<unsigned int>(LB_ERR);
	BOOL			bOutside = FALSE;
								...
								int nImage, int nImage2, DWORD dwFlags, BYTE byMask)
{
	...
	if (lpLBData) {
		...
		nRetValue = __super::SetItemDataPtr(nIndex, lpLBData);
	} // if
...
int CListBoxST::DeleteString(int nIndex)
{
	...
	nRetValue = __super::DeleteString(nIndex);
...
DWORD CListBoxST::GetItemData(int nIndex)
{
	...
	return static_cast<unsigned long>(LB_ERR);
} // End of GetItemData
...
int CListBoxST::Move(int nOldIndex, int nNewIndex, BOOL bSetCurSel)
{
	...
	lpLBData = (STRUCT_LBDATA*)__super::GetItemData(nOldIndex);
	if (lpLBData != (LPVOID)-1L) {
...
{
	DWORD dwAdd = WS_VISIBLE | WS_CHILD | LBS_OWNERDRAWVARIABLE | LBS_HASSTRINGS;
	DWORD dwRem = LBS_MULTIPLESEL;
