...
void CAlarmMachineDlg::UpdateCaption()
{
	...
	if (m_machine->get_is_submachine()) {
		...
		sid.Format(L"%s%06d", fmMachine, m_machine->get_ademco_id());
	}
...
void CAlarmMachineDlg::CheckIfExpire()
{
	...
	if (mins <= 0) {
		...
		s.Format(L"%s\r\n%06d(%s)", e, m_machine->get_ademco_id(), m_machine->get_alias());
		MessageBox(s);
	}
}
	...
	if (TIMER_ID_TRAVERSE_ADEMCO_LIST == nIDEvent) {
		...
		if (--m_nRemoteControlTimeCounter > 0) {
			...
			if (EVENT_QUERY_SUB_MACHINE == m_curRemoteControlCommand) {
				...
				m_machine->SetAdemcoEvent(ER_UNKNOWN, EVENT_OFFLINE, m_machine->get_submachine_zone(),
