...
// selects all cells
void CGridCtrl::SelectAllCells(BOOL bSel)
{
	if (!m_bEnableSelection)
		return;
...
{
	if ((nFlags & GVNI_ALL) == GVNI_ALL) {    // GVNI_ALL Search whole Grid beginning from cell
		...
		for (; row < GetRowCount(); row++) {
			...
			col = GetFixedColumnCount();
			   ...
			   (nFlags & GVNI_TORIGHT) == GVNI_TORIGHT) {   // GVNI_AREA Search Grid beginning from cell to Lower-Right of Grid
