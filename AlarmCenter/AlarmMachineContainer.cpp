...
afx_msg LRESULT CAlarmMachineContainerDlg::OnBnclkedEx(WPARAM wParam, LPARAM lParam)
{
	...
	if (m_bSubmachineContainer) {
		...
		machine = core::alarm_machine_manager::get_instance()->GetMachine(lParam);
	}
...
void CAlarmMachineContainerDlg::Refresh()
{
	...
	list.clear();
	//m_curGroupInfo->SortDescendantMachines(group_manager::get_instance()->get_cur_sort_machine_way());
...
afx_msg LRESULT CAlarmMachineContainerDlg::OnMsgAdemcoevent(WPARAM wParam, LPARAM /*lParam*/)
{
	...
	if (machine) {
		...
		auto group = core::group_manager::get_instance()->GetGroupInfo(group_id);
