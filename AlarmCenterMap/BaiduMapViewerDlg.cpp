...
BOOL CBaiduMapViewerDlg::OnInitDialog()
{
	...
	SetWindowText(TR(IDS_STRING_IDD_DIALOG_PICK_MACHINE_COOR));
	SET_WINDOW_TEXT(IDC_CHECK_AUTO_ALARM, IDS_STRING_IDC_Have_Automatic_Alarm_When_Prompted);	//有报警时自动提示
	SET_WINDOW_TEXT(IDC_BUTTON_SHOW_PATH, IDS_STRING_IDC_BUTTON_SHOW_PATH);
	SET_WINDOW_TEXT(IDC_BUTTON_SHOW_MAP, IDS_STRING_IDC_According_To_The_Map);		//显示地图
	SET_WINDOW_TEXT(IDC_CHECK_AUTO_ALARM2, IDS_STRING_IDC_New_Industry_In_Automatically_Refresh);		//有新警情时自动刷新
	SET_WINDOW_TEXT(IDC_BUTTON_CLEAR_CMB, IDS_STRING_IDC_BUTTON_CLEAR_CMB);
	SET_WINDOW_TEXT(IDC_BUTTON_AUTO_LOCATE, IDS_STRING_IDC_BUTTON_AUTO_LOCATE);
	SET_WINDOW_TEXT(IDC_BUTTON_SET_PT, IDS_STRING_IDC_BUTTON_SET_PT);
	SET_WINDOW_TEXT(IDOK, IDS_OK);

	SET_WINDOW_TEXT(IDC_STATIC_1, IDS_STRING_IDC_STATIC_124);
	SET_WINDOW_TEXT(IDC_STATIC_2, IDS_STRING_IDC_STATIC_125);
	SET_WINDOW_TEXT(IDC_STATIC_4, IDS_STRING_IDC_STATIC_123);
